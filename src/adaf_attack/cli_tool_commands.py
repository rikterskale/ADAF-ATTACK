    ...
    *,
    ...
    def tool_graph(
        ...
        try:
            ...
            error = ActionableError(
                "GRAPH_NOT_FOUND", str(exc), "Pass a valid saved graph.json file."
            )
    ...
    def tool_evidence_import(
        ...
        try:
            ...
            error = ActionableError(
                "INPUT_FILE_INVALID", str(exc), "Check the session, source path, and JSON format."
            )
            emit_error(ctx, error)
            raise typer.Exit(code=error.exit_code) from exc
        emit(
    ...
    def tool_scope(
        ctx: typer.Context, plan: Path = typer.Argument(..., help="YAML scope or engagement plan.")
    ) -> None:
        ...
        try:
            ...
            error = ActionableError(
                "ENGAGEMENT_PLAN_INVALID", str(exc), "Correct the YAML scope document and retry."
            )
            emit_error(ctx, error)
            raise typer.Exit(code=error.exit_code) from exc
        emit(
    ...
    def tool_verify(
        ...
        evidence: list[str] = typer.Option(
            [], "--evidence", help="Evidence reference; repeat as needed."
        ),
    ) -> None:
        ...
        try:
            ...
            error = ActionableError(
                "UNKNOWN_FINDING", str(exc), "Pass a writable session and a valid finding ID."
            )
            emit_error(ctx, error)
            raise typer.Exit(code=error.exit_code) from exc
        emit(
    ...
    def tool_detect(
        ...
        try:
            ...
            error = ActionableError(
                "INPUT_FILE_INVALID", str(exc), "Pass a completed session directory."
            )
            emit_error(ctx, error)
            raise typer.Exit(code=error.exit_code) from exc
        emit(
            ...
            payload,
            Panel(
                f"Detection hypotheses: {payload['count']}\nStatus: review required",
    ...
    def credential_inventory(
        ...
        if missing:
            error = ActionableError(
                ...
                "Pass completed session directories.",
    ...
    def what_if(
        ...
        try:
            ...
            error = ActionableError(
                "GRAPH_NOT_FOUND", str(exc), "Pass a valid graph.json and simulation filter."
            )
            emit_error(ctx, error)
            raise typer.Exit(code=error.exit_code) from exc
        emit(
