...
@register_capability(
    id="rodc-delegation",
    summary="Enumerate RODC password-replication and delegation exposure",
    category="enumeration",
    tags=("rodc", "delegation", "krbtgt"),
)
class RodcDelegation:
    def run(
        ...
        for entry in conn.entries:
            result["rodc_accounts"].append(
                {"sam": str(entry.sAMAccountName), "dn": str(entry.distinguishedName)}
            )
        conn.search(
            base_dn,
        ...
        for entry in conn.entries:
            ...
            constrained = (
                [str(x) for x in (entry["msDS-AllowedToDelegateTo"].value or [])]
                if entry["msDS-AllowedToDelegateTo"]
                else []
            )
