...
def evidence_impact_map(session: Path) -> dict[str, Any]:
    ...
    assets = sorted({str(node.get("id")) for node in nodes if isinstance(node, dict) and node.get("id")})
    mapping = []
    for finding in findings:
        mapping.append({
            ...
            "evidence": finding.get("evidence") or finding.get("category") or "session artifact",
            "affected_assets": assets[:10],
            "impact": "Potential privilege, credential, or control-path exposure; validate before action.",
        })
    return {"ok": True, "session": str(session), "map": mapping, "count": len(mapping), "offline": True}
...
def zero_noise_investigation(session: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "available_actions": ["filter findings", "rank paths", "compare sessions", "generate reports"],
    }
...
def executive_story(session: Path) -> dict[str, Any]:
    ...
    highest = next((level for level in ("critical", "high", "medium", "low") if severity.get(level)), "none")
    narrative = (
...
def confidence_report(session: Path) -> dict[str, Any]:
    ...
    payload = json.loads((session / "findings.json").read_text(encoding="utf-8")) if (session / "findings.json").is_file() else {}
    ...
    for finding in findings if isinstance(findings, list) else []:
        if not isinstance(finding, dict):
            continue
        value = str(finding.get("confidence") or ("medium" if finding.get("evidence") else "unknown")).lower()
        counts[value] += 1
        if value in {"unknown", "low"}:
            review.append(str(finding.get("id") or finding.get("title") or "finding"))
    return {"ok": True, "session": str(session), "confidence_counts": dict(counts), "needs_more_evidence": review, "quality": "strong" if not review else "review-needed"}
...
def deliverables_manifest(session: Path) -> dict[str, Any]:
    ...
    files = sorted(str(path.relative_to(session)) for path in reports.glob("*") if path.is_file()) if reports.is_dir() else []
    return {
        "session": str(session),
        "available": files,
        "expected": ["reports/executive.html", "reports/technical.html", "reports/remediation.html", "redacted-evidence.zip"],
        "ready": bool(files),
