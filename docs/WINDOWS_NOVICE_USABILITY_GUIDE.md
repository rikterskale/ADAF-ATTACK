...
known_limitations: ["No clean windows journey", "No authorized AD lab"]
---

# ADAF-ATTACK Windows Novice Usability Guide
...
Package metadata claims Windows support with Python 3.11 through 3.13. This complete clean path was not runtime validated.
...
```powershell
.\.venv\Scripts\python.exe -m adaf_attack.cli --version
```
...
```powershell
.\.venv\Scripts\python.exe -m adaf_attack.cli doctor
```
...
```powershell
.\.venv\Scripts\python.exe -m adaf_attack.cli list-capabilities
```
...
- `WIN-CMD-001`: `python --version`
- `WIN-CMD-002`: `git --version`
- `WIN-CMD-003`: `py -3.11 -m venv .venv`
- `WIN-CMD-004`: `.\.venv\Scripts\python.exe -m pip install -e ".[full]"`
- `WIN-CMD-005`: `.\.venv\Scripts\python.exe -m adaf_attack.cli --version`
- `WIN-CMD-006`: `.\.venv\Scripts\python.exe -m adaf_attack.cli doctor`
- `WIN-CMD-007`: `.\.venv\Scripts\python.exe -m adaf_attack.cli list-capabilities`
- `WIN-CMD-008`: `Remove-Item -Recurse -Force .venv`
